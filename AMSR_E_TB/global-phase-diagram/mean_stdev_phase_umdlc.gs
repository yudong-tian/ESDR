...
* the following does not make much visual difference. Smaller values more transparent.
transp=0.7
...
*     'set ccolor 22'
     ...
     'set cmark 'marks.lc
      col=lc+19
     'set ccolor 'col
* trim out-of-frame dots
     'define mean2=maskout(mean, 70-mean)'
     'define stdev2=maskout(stdev.2, 20-stdev.2)'
     'define lcmask=maskout(vegtype'lc'.4(t=1), vegtype'lc'.4(t=1)-300)'
     'd maskout(mean2, lcmask);maskout(stdev2, lcmask)' 
     lc=lc+1
     endwhile
     'draw xlab MPDI mean'
...
'gxyat -x 2400 -y 2000 phase-'sn.is'-'ps.ip'-lcmask.png' 
'gxyat -x 1200  -y 1000 phase-'sn.is'-'ps.ip'-lcmask-sm.png' 
