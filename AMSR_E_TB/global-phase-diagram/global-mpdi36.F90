...
! for JJA 2003-2011 and DJF 2002-2009. 
    ...
    Do ip = 1, 2  ! A and D passes 
      ...
      pass = oflag(ip) 
      open(12, file=trim(ctmp) // "_mpdi36.1gd4r", access="direct",form="unformatted", &
               recl=nc*nr*4) 
      nrec=1

      do iyear=2002, 2010 
        do it=date2esec(iyear, 12, 1, 0, 0, 0), date2esec(iyear+1, 2, 28, 0, 0, 0), dt0 
         call rdtb2(it, tb36h, nc, nr, "36H", pass) 
         call rdtb2(it, tb36v, nc, nr, "36V", pass) 
         call compute_mpdi36(tb36h, tb36v, nc, nr, mpdi36) 

         write(12, rec=nrec) mpdi36
         nrec=nrec+1
        end do ! it
      end do  ! iyear
      close(12) 
      ...
      pass = oflag(ip)
      open(22, file=trim(ctmp) // "_mpdi36.1gd4r", access="direct",form="unformatted", &
               recl=nc*nr*4)
      nrec=1
    ...
    subroutine compute_mpdi36(tb36h, tb36v, nc, nr, mpdi36) 
	integer :: nc, nr, i, j
        real :: tb36h(nc, nr), tb36v(nc, nr)
        real :: mpdi36(nc, nr)
	real :: h36, v36
	...
	Do j=1, nr
	  Do i=1, nc
           h36=tb36h(i, j) 
           v36=tb36v(i, j) 
	   if (h36 > 10 .and. v36 > 10) then 
             mpdi36(i, j) = (v36 - h36) / (v36 + h36) * 600.0
           end if 
         End do 
       End do 
...
! $Id: global-mpdi36.F90,v 1.1 2013/03/12 21:50:23 ytian Exp ytian $ 
