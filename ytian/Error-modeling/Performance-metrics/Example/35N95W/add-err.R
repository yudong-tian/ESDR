...
# $Id: add-err.R,v 1.7 2014/10/10 15:03:53 ytian Exp ytian $ 
# Error model: yi = delta + lambda * xi + epsi


print_metrics <- function(x, y, n, icol) { 
   # x and y are replicated a few times before sending to function from original n points. The replication 
   # makes the time series longer, so random numbers (noise) have more for stable param estimation, without
   # changing the statistics.  When plotted, only n points are needed to be shown. 
    ...
    lines.ts(ty, col="blue", lty=1, lwd=1.2) 
    legend("topright", legend=c("Ref.", "Est."), lty=c(1, 1), cex=1.2, lwd=c(1.2, 1.2), col=c("black", "blue")) 
    # label each panel 
    ...
    abline(delta, lamda, lwd=1.5, col="blue") 
...
x=rep(x, 100)   # enlongate for stable noise generation
...
dev.copy(postscript, "compare-add-err.ps", horizontal=T)
dev.off()
